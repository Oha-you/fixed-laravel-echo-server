...
var Channel = (function () {
    ...
    Channel.prototype.leave = function (socket, channel, reason) {
        if (channel) {
            ...
            if (this.options.devMode)
                log_1.Log.info("".concat(socket.id, " left channel: ").concat(channel, " (").concat(reason, ")"), true);
        }
    };
    ...
    Channel.prototype.joinPrivate = function (socket, data) {
        ...
        this.private.authenticate(socket, data).then(function (res) {
            ...
            _this.io.sockets.to(socket.id).emit('channel_subscribe_success', data.channel, res);
        }, function (error) {
    ...
    Channel.prototype.onJoin = function (socket, channel) {
        if (this.options.devMode)
            log_1.Log.info("".concat(socket.id, " joined channel: ").concat(channel), true);
    };
